...
digraph architecture {
    ...
    subgraph cluster_dispatcher {
        ...
        subgraph cluster_scripts {
            ...
            plt_frag [label="detect_fragments\n(fragment analysis)", fillcolor="#FF655D", fontcolor="white"];
            match_atoms [label="match_atoms\n(structure matching)", fillcolor="#FF655D", fontcolor="white"];
        }

        cli -> con_split [label="uv run", fontsize=10];
        cli -> plt_frag [label="uv run", fontsize=10];
        cli -> match_atoms [label="uv run", fontsize=10];
    }

    // Cross-cluster edges
    plt_frag -> geom [lhead=cluster_library, label="imports", style=dashed, color="#004D40"];
...
Plot and CON-facing scripts pin ~chemparseplot>=1.8~ and ~readcon>=0.7.0~ in
their PEP 723 ~dependencies~ lists so isolated ~uv run~ environments receive
metadata-native CON I/O and the centralized unit-aware plot helpers; the
lightweight core install still omits those packages until a script or
~ensure_import~ path requests them.

The correctness of the PEP 723 metadata is enforced by ~pytest-pep723~
{{{cite(@pytest_pep723)}}}, a pytest plugin developed alongside ~rgpycrumbs~ that
...
digraph dataflow {
    ...
    subgraph cluster_inputs {
        ...
        chemgp_h5 [label="ChemGP\n.h5 / .jsonl", fillcolor="#c8e6c9", shape=note];
        traj_xyz [label="ASE trajectory\n.extxyz / .traj", fillcolor="#c8e6c9", shape=note];
        plumed_hills [label="PLUMED\nHILLS", fillcolor="#c8e6c9", shape=note];
    }
    ...
    subgraph cluster_parse {
        ...
        parse_orca [label="parse.orca", fillcolor="#f8bbd0"];
        parse_hdf5 [label="parse.chemgp_hdf5\nparse.chemgp_jsonl", fillcolor="#f8bbd0"];
        parse_traj [label="parse.trajectory", fillcolor="#f8bbd0"];
        parse_plumed [label="parse.plumed", fillcolor="#f8bbd0"];
    }
...
The suite is three packages with a fixed role split (JOSS scope as implemented
in the 1.8 / 1.4.2 line on PyPI):

- *~rgpycrumbs~* -- computational kernels (JAX surface fitting, IRA alignment,
  Hermite interpolation) and the PEP 723 / ~uv~ CLI dispatcher that orchestrates
  eOn visualization scripts (~plt-neb~, ~plt-neb-stitch~, ~plt-min~, ~plt-saddle~,
  ~gen-dimer~, ~plt-kmc~). Plot CLIs are thin orchestrators; they do not own a
  second NEB render implementation.
- *~chemparseplot~* {{{cite(@chemparseplot)}}} -- parsers (eOn ~.dat~ / ~.con~ via
  ~readcon>=0.7~ with CON JSON metadata preferred over legacy tables, ORCA,
  ChemGP HDF5/JSONL, PLUMED, ASE trajectories), unit-aware plotting APIs
  (profiles, landscapes, structure strips, themes), and ~stitch_neb_segments~
  for multi-segment NEB bands. It does *not* implement heavy numerics; those
  stay in ~rgpycrumbs~.
- *~pychum~* -- input generation only (ORCA blocks and eOn templates from TOML);
  not a parser or plotter.

Optional peers such as ~chemparseplot>=1.8~ and ~readcon>=0.7~ are declared in
PEP 723 script blocks and in ~ensure_import~'s dependency map so ~uv~ can
materialize them on demand. Conda-only binaries (~ira_mod~, ~tblite~, ~ovito~)
remain explicit/pixi installs. ChemGP {{{cite(@chemgp)}}} supplies the
GP-accelerated optimization loop that produces the trajectories these tools
analyze.
...
- 2D NEB visualization via RMSD projection {{{cite(@goswami2026nebviz)}}}, with
  reproduction package ~nebviz_repro~
- ChemGP {{{cite(@chemgp)}}}, a Rust-based GP-accelerated optimizer for saddle point
