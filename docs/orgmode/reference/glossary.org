...
Opt-in plot TOML key (default false). When true, chemparseplot evenly
subsamples dense landscape observations for the GP surface *fit* only
(endpoints kept). Set under ~[shared]~ or ~[min]~ / ~[neb]~; not a CLI flag.

* SurfaceFitConfig

chemparseplot dataclass for TOML-friendly surface-fit knobs
(~auto_thin~, ~max_surface_points~). Built via ~from_mapping~ or kwargs;
passed as ~surface_fit=~ to ~plot_landscape_surface~.

* plot.toml

Declarative eOn plot config for ~plt-neb~, ~plt-min~, and ~plt-saddle~
(~--config~). Shared style under ~[shared]~; command inputs under
~[neb]~ / ~[min]~ / ~[saddle]~.

* Energy representation

MethodsX landscape: metric plane (RMSD ~(r, p)~, rotated ~(s, d)~, or
landfold ~(s1, s2)~) plus potential energy and optional ~F_parallel~
synthetic gradients. Dispatch: ~rgpycrumbs landfold plot-energy~.

* landfold.fes.v1

Versioned landfold free-energy result: bin centres ~x~ / ~y~, arrays
~free_energy~ and ~density~ shaped ~(ny, nx)~, plus ~kt~. Produced by
~fes_xy_result~ or ~landfold fes --csv~. chemparseplot
~parse.landfold~ loads it; ~plot.landfold.plot_fes~ fits the NEB GP
landscape. Dispatch: ~rgpycrumbs landfold plot-fes~.
