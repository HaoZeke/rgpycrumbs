...
.. mermaid::
   ...
   flowchart LR
     OBS[x, y, optional gradients] --> MODEL[get_surface_model]
     MODEL --> OPT[hyperparameter fit]
     OPT --> PRED[predict / predict_var]
     PRED --> PLOT[chemparseplot contourf]

.. note::

   Callers in chemparseplot may opt into observation thinning via
   ``SurfaceFitConfig(auto_thin=True)`` before the model is built. That step is
   **off by default** and lives above this module.
#+end_export
