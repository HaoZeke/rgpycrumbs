...
Requires eOn output with ~write_movies=true~ which produces
~minimization.dat~ (structured per-iteration metrics) and ~minimization~
...
| ~minimization.dat~ | Yes | Per-iteration metrics (TSV) |
