...
*TOML-only* (default ~auto_thin = false~). See [[file:plot_config.org][eOn plot TOML config]].
...
|--------+---------+-------------|
| ~--config~ | (none) | TOML plot config (~[shared]~ / ~[neb]~) |
...
| ~--mmf-peaks~ | Off | Overlay OCI-NEB/RONEB refinement samples |
