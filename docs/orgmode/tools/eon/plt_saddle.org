...
When available, ~climb.dat~ remains the preferred source for structured
per-iteration metrics; for metadata-rich full CON movies, =plt-saddle= can also
reconstruct the table directly from per-frame CON metadata.
...
| ~climb.dat~ | Preferred | Per-iteration metrics (TSV); falls back to per-frame CON metadata when absent |
