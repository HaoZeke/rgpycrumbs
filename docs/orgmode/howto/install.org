...
honor an optional CycloneDX SBOM for PyPI pins:

#+begin_src bash
export RGPYCRUMBS_SBOM=/path/to/stack.cdx.json   # e.g. eb-stack --sbom-out
rgpycrumbs --sbom "$RGPYCRUMBS_SBOM" eon plt-neb ...
#+end_src

Only ~pkg:pypi/...~ components become ~name==version~ constraints; EasyBuild
~pkg:generic/...~ inventory rows are skipped. No SBOM → floating PEP 723 / AUTO_DEPS.
