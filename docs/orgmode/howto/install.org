...
export RGPYCRUMBS_LOCK=/path/to/uv.lock
# or: RGPYCRUMBS_LOCK=pylock.toml
# or: RGPYCRUMBS_SBOM=stack.cdx.json   # alias for --sbom
rgpycrumbs --lock "$RGPYCRUMBS_LOCK" eon plt-neb ...
#+end_src
...
#+begin_src bash
# global
mkdir -p ~/.config/rgpycrumbs
cp docs/examples/rgpycrumbs.config.toml ~/.config/rgpycrumbs/config.toml

# project (overrides global; walk-up from CWD)
cp docs/examples/rgpycrumbs.config.toml ./rgpycrumbs.toml
#+end_src
...
Precedence: CLI ~--lock~ → env ~RGPYCRUMBS_LOCK~ → project TOML →
~/.config/rgpycrumbs/config.toml~. Optional ~RGPYCRUMBS_CONFIG~ merges last among
files.
