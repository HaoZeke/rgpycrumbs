...
.. grid:: 2
   ...
   .. grid-item-card:: Surfaces
      JAX kernel models for PES-style interpolation.

   .. grid-item-card:: eOn library + CLI plots
      ``from rgpycrumbs.eon import plot, plot_neb``; TOML for surface-fit knobs.
#+end_export
...
No feature extras. Use ~export RGPYCRUMBS_AUTO_DEPS=1~ (library) or the
CLI ~uv~ PEP 723 path. Install ~jax~/~scipy~/~ase~ yourself only if you
want them preloaded in the host env.
...
This resolves the full project stack, including pixi-only packages like ~tblite~
and ~ira_mod~, plus heavier optional tools such as ~ovito~.
...
Prefer the Python API for notebooks and scripts (same pipelines as the CLI).
Heavy deps (~chemparseplot~, ~jax~, ~xyzrender~, ~adjustText~) stage through
~ensure_import~; library entry points enable ~RGPYCRUMBS_AUTO_DEPS~ by default.
...
Use ~config="plot.toml"~ for strip / surface-fit settings. See
[[file:tools/eon/plot_config.org][plot TOML config]] and the eOn tool pages under Tools.
