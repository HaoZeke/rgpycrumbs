...
def _get_adjust_text():
    """Get adjustText module."""
    global _adjust_text
    if _adjust_text is None:
        _adjust_text = ensure_import("adjustText")
    return _adjust_text
...
@click.command()
@click.option("--input-dat-pattern", default=DEFAULT_INPUT_PATTERN, help="Glob pattern for input data files.")
@click.option("--input-path-pattern", default=DEFAULT_PATH_PATTERN, help="Glob pattern for input path files.")
@click.option("--con-file", type=click.Path(exists=True, path_type=Path), default=None, help="Path to .con trajectory file.")
@click.option("--source", type=click.Choice(["eon", "traj", "hdf5"]), default="eon", help="Data source.")
@click.option("--input-h5", type=click.Path(exists=True, path_type=Path), default=None, help="Path to ChemGP NEB HDF5 file.")
@click.option("--input-traj", type=click.Path(exists=True, path_type=Path), default=None, help="Path to extxyz trajectory file.")
@click.option("--plot-type", type=click.Choice(["profile", "landscape"]), default="profile", help="Type of plot to generate.")
@click.option("--landscape-mode", type=click.Choice(["path", "surface"]), default="surface", help="For landscape plot.")
@click.option("--project-path/--no-project-path", is_flag=True, default=True, help="Project landscape coordinates.")
@click.option("--plot-structures", type=click.Choice(["none", "all", "crit_points"]), default="none", help="Structures to render.")
@click.option("--output-file", "-o", type=click.Path(path_type=Path), default=None, help="Output image filename.")
@click.option("--figsize", nargs=2, type=(float, float), default=(5.37, 5.37), help="Figure width, height in inches.")
...
@click.option("--cache-file", type=click.Path(path_type=Path), default=Path(".neb_landscape.parquet"), help="Parquet cache file.")
@click.option("--force-recompute", is_flag=True, default=False, help="Force re-calculation of RMSD.")
@click.option("--show-pts/--no-show-pts", is_flag=True, default=True, help="Show data points on surface.")
@click.option("--show-legend", is_flag=True, default=False, help="Show colorbar legend.")
@click.option("--surface-type", default="grad_imq", help="Surface interpolation method (grad_imq, grad_matern, grad_imq_ny, rbf).")
@click.option("--landscape-path", type=click.Choice(["final", "all", "none"]), default="final", help="Which path steps to overlay on landscape.")
def main(
    input_dat_pattern,
    input_path_pattern,
    con_file,
    ...
    if plot_type == "landscape":
        ...
        if landscape_mode == "surface":
            ...
            # Simple smoothing heuristic for RBF
            rbf_smoothing = float(len(r_all)) * 0.01 if surface_type == "rbf" else None
            
            plot_landscape_surface(
                ax,
                r_all, p_all, gr_all, gp_all, z_all,
                method=surface_type,
        ...
        if show_legend:
            ...
            norm = mpl.colors.Normalize(vmin=df_final["z"].min(), vmax=df_final["z"].max())
        ...
        if has_strip and ax_strip is not None and atoms_list:
            ...
            if plot_structures == "crit_points":
                ...
                if len(atoms_list) > max_structures:
                    indices = np.linspace(0, len(atoms_list) - 1, max_structures, dtype=int)
            ...
            plot_structure_strip(
                ax_strip, strip_atoms, strip_labels,
                zoom=0.3, rotation="0x,90y,0z",
                theme_color="black", max_cols=6
            )
        ...
        log.info("Profile plot not yet implemented in thin wrapper - use chemparseplot directly")
        plt.close(fig)
